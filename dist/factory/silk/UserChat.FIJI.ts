...
export default async function(client: GMDIBot) {
  ...
  let data: [string, UserChatInterface][] = Object.entries(client.database.get("userChatRate"));
  ...
  for (const [id, content] of data) {
    ...
    if (!content?.sum?.week) {
      client.database.set(`userChatRate.${id}.sum.week`, [content.sum.today]);
    } else {
      if (content.sum.week.length >= 7) {
        client.database.push(`userChatRate.${id}.sum.week`, content.sum.week.slice(1).push(content.sum.today));
        currentOverload = true;
      } else {
        client.database.push(`userChatRate.${id}.sum.week`, content.sum.today);
      };
    };

    // reset
    client.database.set(`userChatRate.${id}.sum.today`, 0);
    ...
    client.database.set(`userChatRate.${id}.probability`, res);
    ...
    if (arrayOfAvg.length === 7 && res <= Config.userChat.meta.threshold) {
      if (!content.warning?.current) {
        ...
        client.database.set(`userChatRate.${id}.warning`, varyObj);
      }
